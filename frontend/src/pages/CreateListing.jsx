...
export default function CreateListing() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await carsAPI.create(carData)
      const carId = res.data.id

      if (images.length > 0) {
        const formData = new FormData()
        images.forEach((img) => formData.append('files', img.file))
        await carsAPI.uploadImages(carId, formData)
      }

      navigate('/moji-oglasi')
    } catch (err) {
      setError(err.response?.data?.detail || 'Greška pri kreiranju oglasa')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">Postavi novi oglas</h1>
        <p className="text-gray-500 text-sm">Popunite informacije o vašem automobilu</p>
      </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
            {images.map((img, i) => (
              <div key={i} className="relative aspect-square rounded-lg overflow-hidden bg-gray-100">
                ...
                <button type="button" onClick={() => removeImage(i)} className="absolute top-1 right-1 bg-red-500 text-white p-1 rounded-full border-0 cursor-pointer"><X size={12} /></button>
              </div>
            ))}
            {images.length < 10 && (
